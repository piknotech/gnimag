...
final class TapPredictor: TapPredictorBase {
    ...
    private struct Distances {
        ...
        static let afterCollision = 0.15
    ...
    private var pastEvents = FixedSizeFIFO<Event>(capacity: 1)
    ...
    init(playfield: Playfield, tapper: SomewhereTapper, timeProvider: TimeProvider, gameModel: GameModel) {
        ...
        super.init(tapper: tapper, timeProvider: timeProvider, tapDelayTolerance: .absolute(0))
    ...
    private func updateTokens(realTime: Double) {
        if let latestChange = gameModel.prism.mostRecentChange {
            // Remove the first token
            if !tokens.isEmpty { tokens.removeFirst() }

            // After leaving unsure, remove all tokens
            if latestChange.from == .unsure { tokens.removeAll() }
        }
    ...
    private func tapPerformed(tap: PerformedTap) {
        if delay == 0 { return } // Not yet initialized
