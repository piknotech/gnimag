...
const color_lookup_t color_dict[MAX_COLORS] = {
  { 'A', 'A', "101", "ff0000", "723939" }, // red
  { 'B', 'B', "104", "0000ff", "393972" }, // blue
  { 'C', 'C', "103", "eeee00", "6e6e39" }, // yellow
  { 'D', 'D',  "42", "008100", "395539" }, // green
  { 'E', 'E',  "43", "ff8000", "725539" }, // orange
  { 'F', 'F', "106", "00ffff", "397272" }, // cyan
  { 'G', 'G', "105", "ff00ff", "723972" }, // magenta
  { 'H', 'H',  "41", "a52a2a", "5f4242" }, // maroon
  { 'I', 'I',  "45", "800080", "553955" }, // purple
  { 'J', 'J', "100", "a6a6a6", "5f5e5f" }, // gray
  { 'K', 'K', "107", "ffffff", "727272" }, // white
  { 'L', 'L', "102", "00ff00", "397239" }, // bright green
  { 'M', 'M',  "47", "bdb76b", "646251" }, // tan
  { 'N', 'N',  "44", "00008b", "393958" }, // dark blue
  { 'O', 'O',  "46", "008180", "395555" }, // dark cyan
  { 'P', 'P',  "35", "ff1493", "72415a" }, // pink?
};
...
void game_print_buffer(char* buffer,
  ...
  for (size_t y=0; y<info->size; ++y) {
    for (size_t x=0; x<info->size; ++x) {
      cell_t cell = state->cells[pos_from_coords(x, y)];
      location += sprintf(location, "%s", color_cell_str(info, cell));
    }
...
int game_next_move_color(const game_info_t* info,
  ...
  int last_color = state->last_color;
    ...
    if (match_id >= 0) {
      ...
      if (options[match_id].dst_flag) {
        ...
        *options[match_id].dst_flag = options[match_id].dst_value;
        ...
        char* endptr;
        g_options.node_bottleneck_limit = (int)strtol(opt, &endptr, 10);
...
char* solve_board(const char* board) {
    ...
    FILE* file = fmemopen((void*)board, strlen(board), "r");
    if (game_read(file, &info, &state)) {
        game_order_colors(&info, &state, 0);

        double elapsed;
        size_t nodes;
        game_state_t final_state;

        char* solution = malloc((info.size + 1) * (info.size + 1) * sizeof(char));
        memset(&solution[0], 0, sizeof(solution));
        
        int result = game_search(&info, &state, 0, &elapsed, &nodes, &final_state);
        if (result == SEARCH_SUCCESS) {
            game_print_buffer(solution, &info, &final_state);
            return solution;
        }
    }
